...
JSON parse_array(const string &str, size_t &offset) {
    ...
    while (offset < str.length()) {
        ...
        if (offset < str.length() && str[offset] == ',') {
            ...
            if (offset >= str.length()) {
                throw std::runtime_error("json parsing: Array: Expected ',' or ']', found EOF");
            }
...
JSON parse_string(const string &str, size_t &offset) {
    ...
    string val;
    if (offset >= str.length()) {
        throw std::runtime_error("json parsing: String: Expected '\"', found EOF");
    }
    offset++;
    if (offset >= str.length()) {
        throw std::runtime_error("json parsing: String: Expected '\"', found EOF");
    }
    for (char c = str[offset]; offset < str.length() && c != '\"'; c = str[offset]) {
        if (c == '\\') {
            offset++;
            if (offset >= str.length()) {
                throw std::runtime_error("json parsing: String: Expected '\"', found EOF");
            ...
            case 'u': {
                ...
                for (unsigned i = 1; i <= 4; ++i) {
                    ...
                    if ((c >= '0' && c <= '9') || (c >= 'a' && c <= 'f') || (c >= 'A' && c <= 'F'))
                        ...
                        throw std::runtime_error("json parsing: Expected hex character in unicode escape, found another");
                    }
                }
                offset += 4;
            } break;
    ...
    if (c == 'E' || c == 'e') {
        ...
        while (offset < str.length()) {
            ...
            if (c >= '0' && c <= '9')
                ...
                if (offset >= str.length()) {
                    throw std::runtime_error("json parsing: Number: Expected a number for exponent, found EOF");
                }
...
JSON parse_next(const string &str, size_t &offset) {
    ...
    throw std::runtime_error("json parsing: Unknown starting character ");
    return JSON();
}
} // namespace
